...
namespace merissu
{
    public class CompProperties_SkandaFeet : CompProperties_AbilityEffect
    {
        public CompProperties_SkandaFeet()
        {
            compClass = typeof(CompAbilityEffect_SkandaFeet);
        }
    }

    public class CompAbilityEffect_SkandaFeet : CompAbilityEffect
    {
    ...
    public class Mote_SkandaTrail : Thing
    {
        ...
        private void ApplyShockwave()
        {
            ...
            for (int dx = -1; dx <= 1; dx++)
            {
                for (int dz = -1; dz <= 1; dz++)
                {
                    ...
                    {
                        Pawn victim = things[i] as Pawn;
                        if (victim == null || victim == caster || victim.Dead) continue;
                        if (victim.Faction != null && !victim.Faction.HostileTo(caster.Faction)) continue;
                        if (!damagedPawns.Add(victim)) continue;
        ...
        private void UpdateCustomMesh(float length)
        {
            ...
            {
                customMesh = new Mesh();
                customMesh.name = "SkandaTrailMesh";
