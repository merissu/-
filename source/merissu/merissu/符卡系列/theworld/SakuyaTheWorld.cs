...
namespace merissu
{
    public class SakuyaMod : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect inRect)
        {
            ...
            listing.CheckboxLabeled(
                "暂停投射物",
                ref Settings.pauseProjectiles);

            listing.CheckboxLabeled(
                "暂停所有动画 (水面/火焰/翅膀)",
                ref Settings.pauseAnimations);
    ...
    public static class YayoAnimation_Compat_Patch
    {
        ...
        public static void Initialize()
        {
            ...
            try
            {
                ...
                {
                    Log.Message(
                        "[Merissu] 未找到 YayoAnimation.AnimationCore，跳过 Yayo 兼容补丁。");
                    return;
                }
                ...
                {
                    Log.Warning(
                        "[Merissu] 找到 AnimationCore，但没有找到 CheckAni。");
                    return;
                }
    ...
    public static class TimeStopManager
    {
        ...
        public static Dictionary<int, FrozenGunState> FrozenGuns =
            new Dictionary<int, FrozenGunState>();

        public static readonly HashSet<int> SpawnedDuringTimeStop =
            new HashSet<int>();
        ...
        public static void ActivateTheWorld(Pawn caster)
        {
            ...
            if (caster.Map != null)
            {
                EnvState.frozenShadowInfo =
                    GenCelestial.GetLightSourceInfo(
                        caster.Map,
                        GenCelestial.LightType.Shadow);

                EnvState.celestialGlow =
                    GenCelestial.CurCelestialSunGlow(caster.Map);

                Vector2 longLat =
                    Find.WorldGrid.LongLatOf(caster.Map.Tile);

                EnvState.dateStringCached =
                    GenDate.DateFullStringAt(
                        EnvState.startTicksAbs,
                        longLat);
            }
            ...
            {
                GenSpawn.Spawn(
                    SakuyaThingDefOf.Sakuya_TimeStopVisual,
                    caster.Map.Center,
                    caster.Map);
            }

            SoundDef theWorldSound =
                SoundDef.Named("theworld");

            if (theWorldSound != null)
            {
                theWorldSound.PlayOneShot(
                    new TargetInfo(
                        caster.Position,
                        caster.Map));
            }
                ...
                caster.health != null)
            {
                foreach (string defName in hediffsToAdd)
                {
                    ...
                    if (def != null &&
                        !caster.health.hediffSet.HasHediff(def))
                    {
        ...
        public static void ResumeTime()
        {
            ...
            try
            {
                if (TimeStopOwner != null &&
                    TimeStopOwner.health != null)
                {
                    Hediff def =
                        TimeStopOwner.health.hediffSet
                            .GetFirstHediffOfDef(
                                HediffDef.Named("the"));
        ...
        {
            if (!IsTimeStopped || thing == null)
                return true;

            if (SpawnedDuringTimeStop.Contains(
                    thing.thingIDNumber))
            {
                return true;
            }
            ...
            if (thing == TimeStopOwner)
                return true;

            IThingHolder parentHolder =
                thing.ParentHolder;

            if (parentHolder is Pawn_ApparelTracker a &&
                a.pawn == TimeStopOwner)
            {
                return true;
            }

            if (parentHolder is Pawn_EquipmentTracker e &&
                e.pawn == TimeStopOwner)
            {
                return true;
            }

            if (thing is Projectile p &&
                p.Launcher == TimeStopOwner &&
                SakuyaMod.Settings != null &&
                !SakuyaMod.Settings.pauseProjectiles)
            {
                return true;
            }
    ...
    public class TimeStopVisual : Thing
    {
            ...
            bool flip = false)
        {
            ...
            float breath =
                (Mathf.Sin(
                    Time.realtimeSinceStartup * 2f) + 1f) *
                0.25f;
    ...
    public class SakuyaTheWorld : Ability
    {
        ...
        public SakuyaTheWorld(Pawn pawn)
            : base(pawn)
        {
        }

        public SakuyaTheWorld(
            Pawn pawn,
            AbilityDef def)
            : base(pawn, def)
        {
    ...
    public static class Patch_UI_Clock_Freeze
    {
            ...
            ref float curBaseY)
        {
            ...
            Widgets.Label(
                rect,
                TimeStopManager.EnvState.dateStringCached);
    ...
    public static class Patch_Shadow_Freeze
    {
        public static bool Prefix(
            Map map,
            GenCelestial.LightType type,
            ref GenCelestial.LightInfo __result)
        {
            if (TimeStopManager.IsTimeStopped &&
                type == GenCelestial.LightType.Shadow)
            {
                __result =
                    TimeStopManager.EnvState.frozenShadowInfo;
    ...
    public static class Patch_Sun_Glow_Freeze
    {
            ...
            ref float __result)
        {
            ...
            {
                __result =
                    TimeStopManager.EnvState.celestialGlow;
    ...
    {
        public static bool Prefix(
            Graphic_Flicker __instance,
            Vector3 loc,
            Rot4 rot,
            ThingDef thingDef,
            Thing thing,
            float extraRotation,
            Graphic[] ___subGraphics)
        {
            if (!TimeStopManager.IsTimeStopped ||
                SakuyaMod.Settings == null ||
                !SakuyaMod.Settings.pauseAnimations)
            ...
            if (___subGraphics != null &&
                ___subGraphics.Length > 0)
            {
                float fireSize = 1f;
                ...
                int frozenIndex =
                    Math.Abs(
                        (TimeStopManager.FrozenTick + seed) %
                        ___subGraphics.Length);

                float sineWave =
                    Mathf.Sin(
                        seed +
                        TimeStopManager.FrozenTime * 15f);
                ...
                Graphic graphic =
                    ___subGraphics[frozenIndex];

                Vector2 originalDrawSize =
                    graphic.drawSize;
        ...
        public static void Postfix()
        {
                ...
                SakuyaMod.Settings.pauseAnimations)
            {
                Shader.SetGlobalFloat(
                    ShaderPropertyIDs.GameSeconds,
                    TimeStopManager.FrozenTime);
            }
        }
    }
    ...
    public static class Patch_Proj_Tick
    {
        ...
        {
            return
                !TimeStopManager.IsTimeStopped ||
                TimeStopManager.IsProtected(__instance);
        }
    }
    ...
    public static class Patch_Tweener_Freeze
    {
            ...
            Pawn ___pawn)
        {
            ...
            return
                ___pawn == null ||
                TimeStopManager.IsProtected(___pawn);
        }
    }

    [HarmonyPatch(
        typeof(PawnDownedWiggler),
        nameof(PawnDownedWiggler.ProcessPostTickVisuals))]
    public static class Patch_DownedWiggler_Freeze
    {
        public static bool Prefix(
            PawnDownedWiggler __instance,
            int ticksPassed)
        {
            if (!TimeStopManager.IsTimeStopped)
                return true;
    ...
    [HarmonyPatch(
        ...
        nameof(PawnRenderTree.AnimationTick),
        MethodType.Getter)]
    public static class Patch_RenderTree_Animation_Freeze
    {
            ...
            ref int __result)
        {
            ...
            if (pawn == null ||
                TimeStopManager.IsProtected(pawn))
            {
                return true;
            }
    ...
    [HarmonyPatch(
        typeof(Pawn_RotationTracker),
        "UpdateRotation")]
    public static class Patch_Rotation_Freeze
    {
        public static bool Prefix(
            Pawn_RotationTracker __instance,
            Pawn ___pawn)
        {
            if (!TimeStopManager.IsTimeStopped)
                return true;
    ...
    public static class Patch_MainTick
    {
        ...
        private static readonly Action<Thing> doTick =
            AccessTools.MethodDelegate<Action<Thing>>(
                AccessTools.Method(
                    typeof(Thing),
                    "Tick"));

        private static readonly Action<Thing> doTickRare =
            AccessTools.MethodDelegate<Action<Thing>>(
                AccessTools.Method(
                    typeof(Thing),
                    "TickRare"));

        private static readonly Action<Thing> doTickLong =
            AccessTools.MethodDelegate<Action<Thing>>(
                AccessTools.Method(
                    typeof(Thing),
                    "TickLong"));
        ...
        public static bool Prefix(
            TickList __instance,
            TickerType ___tickType,
            List<List<Thing>> ___thingLists)
        {
            if (!TimeStopManager.IsTimeStopped)
                return true;

            int interval =
                ___tickType == TickerType.Normal
                    ? 1
                    : ___tickType == TickerType.Rare
                        ? 250
                        : 2000;

            List<Thing> toRegister =
                (List<Thing>)
                thingsToRegisterField.GetValue(__instance);

            List<Thing> toDeregister =
                (List<Thing>)
                thingsToDeregisterField.GetValue(__instance);
                ...
                toRegister.Count > 0)
            {
                     ...
                     i++)
                {
                    ...
                    TimeStopManager.RegisterTimeStopSpawn(
                        thing);
                    ...
                    List<Thing> targetBucket =
                        ___thingLists[
                            bucketIndex % interval];

                    if (!targetBucket.Contains(thing))
                        targetBucket.Add(thing);
                }
            ...
            if (toDeregister != null &&
                toDeregister.Count > 0)
            {
                for (int i = 0;
                     i < toDeregister.Count;
                     i++)
                {
                    Thing thing =
                        toDeregister[i];
                    ...
                    TimeStopManager.SpawnedDuringTimeStop.Remove(
                        thing.thingIDNumber);
            ...
            List<Thing> bucket =
                ___thingLists[
                    Find.TickManager.TicksGame % interval];

            for (int i = bucket.Count - 1;
                 i >= 0;
                 i--)
            {
                ...
                try
                {
                    if (___tickType == TickerType.Normal)
                        doTick(thing);
                    else if (___tickType == TickerType.Rare)
                        doTickRare(thing);
                    else if (___tickType == TickerType.Long)
                        doTickLong(thing);
                }
                catch (Exception e)
                {
    ...
    public static class Patch_Weapon_Aim_Freeze
    {
            ...
            ref float aimAngle)
        {
            ...
            Pawn p =
                (eq.ParentHolder as Pawn_EquipmentTracker)?.pawn;

            if (p == null ||
                TimeStopManager.IsProtected(p))
            {
                return;
            }
            ...
            if (TimeStopManager.FrozenGuns.TryGetValue(
                    gunID,
                    out FrozenGunState state))
            {
            ...
            {
                TimeStopManager.FrozenGuns[gunID] =
                    new FrozenGunState(
                        drawLoc,
                        aimAngle);
            }
